...
Tensor spmm_csr_long(std::vector<int> result_shape, std::vector<int> A_shape,
  ...
  long* SCORCH_RESTRICT A_val = A_values.data_ptr<long>();

  // Get B's level & value arrays
  int B0_size = B_shape[0];
  int B1_size = B_shape[1];
  long* SCORCH_RESTRICT B_val = B_values.data_ptr<long>();

  // Initialize result value array
  int C_capacity = C0_size * C1_size;
  long* SCORCH_RESTRICT C_values = (long *)malloc(sizeof(long) * C_capacity);
  memset(C_values, 0, sizeof(long) * C_capacity);
  ...
  if (residual_k_start < B1_size) {
    for (int i = 0; i < A0_size; i++) {
      ...
      long *accum_c = new long[tile_k_width]();
      int pA1_end = A1_pos[i + 1];
  ...
  torch::Tensor C_values_torch = torch::from_blob(
      C_values, {C_capacity}, C_values_deleter, torch::kInt64); // Changed to kInt64
