...
- Latency-policy revision: 2026-07-13; correctness and generality take priority,
  with 1.10x retained as the default measurement target and investigation
  threshold rather than an automatic rejection boundary
- Companion implementation handoff:
  [`COMPILER_IR_REFACTOR_HANDOFF.md`](COMPILER_IR_REFACTOR_HANDOFF.md)

This is the canonical north-star design. It documents the final architecture,
the responsibilities and invariants of every stage, the full migration sequence,
and the criteria for retiring the legacy compiler path. It is intentionally
broader than the first implementation task.

## Binding Review Decisions

The following decisions are resolved and are part of this design, not deferred
...
- Analyses are pure functions over immutable IR and return ID-keyed side tables.
  ...
  fingerprint-keyed analysis cache and no preserve/invalidate protocol. Add
  caching only if stage-latency profiles demonstrate a material bottleneck.
- LoopIR feasibility for sparse merging is proved before its schema is frozen.
  A mandatory spike must hand-author candidate LoopIR for CSR SpMV intersection
  and sparse elementwise-add union and execute both through a small reference
  interpreter.
- The investment north star is code-generation parity with handwritten prebuilt
  schedules: a new schedule should be expressible as `LoopPlan` plus typed passes
  and perform competitively without a new algorithm-specific C++ kernel.
- Cutover and legacy deletion compare the new pipeline against legacy generated
  kernels only. Parity with handwritten prebuilt kernels is validated at the
  selector level in a dedicated post-cutover milestone (Phase 8.5); a shortfall
  there triggers a scope review, never retention of the legacy compiler.
- Correctness, fail-closed behavior, and general compiler structure take
  precedence over minimizing Python compiler latency. Compile-latency budgets
  are phase-local regression monitors: the Phase 0 ratio remains the default
  target and investigation threshold, but a modest, measured, stage-attributed
  increase may be accepted when it buys general typed infrastructure. Do not
  weaken validation or add format-, expression-, or kernel-specific shortcuts
  merely to hit the target. Material or unexplained regressions still block a
  phase. Full internal IR verification is enabled by default in tests/debug
  builds and disabled by default in the production JIT path; cheap public-input
  validation remains on.
- Generated-kernel pass/fail is calibrated against same-binary A/A noise-floor
  controls on each machine, never against fixed ratio constants; a valid gate
  must pass when applied to its own A/A control. Byte-identical emission across
  the grid corpus under identical build flags waives runtime benchmarks;
  structural activation tests are never waived.
...
- `ScheduledCIN` is only a frozen transitional carrier for normalized CIN and a
  verified `LoopPlan`. It is not a fourth IR and has no independent node model.
- Concurrent compilation is a desirable hardening property, not a migration
  success gate.
- Phase 3 plus the sparse feasibility spike ends in an explicit go/no-go review.
  Stopping there must still leave the current compiler safer and more
  maintainable.

## Executive Decision

Rebuild the compiler incrementally around immutable artifacts at pass boundaries,
stable identities, structured IRs, explicit analysis and scheduling artifacts,
stage verifiers, and fail-closed diagnostics.
...
This will be delivered through a strangler migration. The legacy pipeline remains
available until replacement paths have structural, code-generation, and numerical
differential coverage. There will be no flag-day rewrite.

## Motivation

Scorch's compiler contains valuable sparse-iteration and scheduling logic, but
stage ownership is implicit:

- CIN owns mutable backreferences and name-based identity.
- Scheduling mutates CIN and attaches undeclared metadata.
- Sparse scheduling is split between CIN and post-LLIR transformations.
- Iteration analysis and lowering recursively call into each other.
- LLIR stores C++ expressions inside symbol names and raw statements.
- Passes parse generated C++ spelling to recover semantics.
- ABI construction, result assembly, optimization, parallel policy, and semantic
  lowering coexist in `CINLowerer`.
- Unsupported nodes sometimes fail open and surface as downstream C++ errors.
...
The goal is not abstraction for its own sake. The goal is to make every compiler
...
SpMM implementations and their tiling selector. That is a useful blast-radius
boundary during migration, but it also exposes the current compiler's limiting
cost: schedules such as tile-j and tile-ijk became handwritten C++ kernels
because the generated path could not express them competitively.

The falsifiable payoff for this refactor is therefore not just legacy-code
deletion. After migration, a schedule should be implementable as a `LoopPlan`
plus reusable typed transformations and reach parity with its handwritten
...
This north star validates the investment; it does not gate cutover. Deleting
the legacy compiler requires only that the new pipeline match the kernels the
legacy compiler generates today, a compiler-versus-compiler comparison. Parity
with handwritten prebuilt kernels is a strictly higher bar, and keeping the
legacy compiler alive contributes nothing toward closing any remaining gap.
Prebuilt parity is also a system-level claim: the runtime tiling selector armed
with generated kernels must match the selector armed with prebuilt kernels,
because prebuilt schedules only execute on shapes where the selector's probe
chooses them. Per-cell dominance over a hand-tuned kernel on shapes the
selector would never route to it is not the target.
...
8. Cache identity is derived from canonical artifacts and explicit compile
   options, not incidental object representation.
...
- Adding new sparse formats or kernel optimizations during the IR migration.
  Re-expressing existing tile-j/tile-ijk schedules to prove codegen parity is
  migration validation, not a new optimization experiment.
...
Builders may use local mutation while constructing an artifact. Once finalized,
an artifact passed to another stage is immutable. A pass returns a new artifact
and cannot modify its input.

This is more practical than requiring every temporary object to be frozen, while
still preventing hidden aliasing across compilations and passes.
Transformations rebuild only changed paths, normally with `dataclasses.replace`;
unchanged frozen nodes may be structurally shared safely.
...
Transformations creating new entities assign new IDs and retain provenance such
as `derived_from`, `split_from`, or `lowered_from`.
...
an empty statement list, ignored post-op, placeholder C++ text, or silently
degraded schedule.

### Explicit target boundary

CIN and LoopIR do not know about Torch C++ field names, `torch::from_blob`, OpenMP
pragma spelling, or native return structs. Those belong to CxxIR and the
`TorchCppABI` layer.
...
The context must not become a replacement bag of phase-dependent fields. State
that affects semantics belongs in an explicit artifact.
...
The exact Python representation may be simpler initially. The important contract
is explicit ownership and deterministic serialization. Fingerprints are computed
on demand for persistent build/cache identity or differential comparison; they
are not mandatory fields carried through every pass.
...
- no implicit output or hidden `_assignment` side effects;
...
for the small IRs generated by current Scorch operations, this favors correctness
and simplicity over speculative reuse.

There is deliberately no analysis-result cache and no pass-level
preserve/invalidate declaration. Caching may be proposed later only with stage
profiles showing that recomputation is material, an explicit cache key, and
tests proving that stale results cannot be observed.
...
verified plan together through legacy seams. Once CIN plus `LoopPlan` lower
directly to LoopIR, this carrier should disappear rather than accrete into
another IR.

## Stage 4: LoopIR

LoopIR is the central future design. It represents executable iteration semantics
without committing to C++/OpenMP/Torch ABI spelling.
...
passes. It is a verified stage/state of the same LoopIR node model, not a fourth
...
unchanged loops and attach provenance to moved/split loops.

#### Affine tiling

Split one loop into outer/inner loops with explicit bounds and ragged-tail
...
In addition to ordinary LoopIR checks:
...
Existing post-lowering optimizations migrate into explicit passes over structured
LoopIR or CxxIR, depending on the semantics they require.
...
Required analyses are invoked as pure computations for the current input. Passes
do not declare preservation or invalidation in the initial architecture.
...
It replaces ABI construction and result assembly currently embedded in
...
This specification is the input to the centralized runtime cache/compiler, not
an ad-hoc hash of selected strings.
...
Persistent build/cache identity and differential comparison must not depend on
memory addresses, dictionary ordering, or allocation-order IDs. Canonical
serialization is computed when those uses require it and:
...
`str()` and `repr()` are for humans, not cache identity.
...
They are disabled by default in production/release JIT compilation so the safety
discipline does not become hidden per-compile latency. Cheap validation at public
and trust boundaries remains enabled in all modes.
...
- identify the first verifier or differential stage that diverged.
...
## Performance and Latency Gate Protocol

Performance is checked where behavior changes, not postponed until cutover.

### Generated-kernel gate

The repository's codegen-parity grid runs on both reference machines: Apple M5
and x86/redwood. Every cell must remain numerically correct.

Noise is calibrated before changes are judged. Every gate run is accompanied by
a same-binary A/A control: the baseline measured against itself on the same
grid and machine with the same interleaved repeated-measurement methodology.
The control establishes, per cell and per machine, the ratio spread that "no
change" produces. This is the noise-floor-control methodology already used for
the tiling selector's no-regression proof (v2-vs-v2 on both machines).

Pass/fail is defined against that control, not against fixed constants:

- a cell fails when its new/old median runtime ratio exceeds the band its A/A
  control establishes for that cell;
- a machine fails when its ratio geomean falls outside the A/A geomean band;
- thresholds must account for the number of cells compared at once and satisfy
  one calibration requirement: the gate, applied to the A/A control itself,
  passes. A rule that a same-binary comparison cannot pass is measuring noise,
  not regressions.
...
rerun-until-it-passes: repeating a measurement until a noisy cell dips below a
threshold selects on noise instead of establishing safety. A rerun is
...
invariant-factor hoisting, and parallel zero-fill is not complete until its
structural activation tests and, where emission differs, the two-machine
before/after benchmarks land together.

Byte-identical emission waives the runtime benchmark. If a change produces
byte-for-byte identical C++ for every cell of the grid corpus under identical
build flags, the compiled kernels are identical, and benchmarking a binary
against itself can only produce noise that demands investigation of nothing.
The PR must instead include the byte-identical evidence across the corpus and
the structural activation tests, which are never waived. The runtime benchmark
...
curated corpus includes small dense, reduction, CSR intersection, and sparse
union cases. Phase 2 separately measures empty pass/artifact plumbing, whose p95
incremental overhead must be at most 1 ms. In production/release mode, every
corpus category should target end-to-end Python compiler p50 and p95 at or below
1.10 times its Phase 0 baseline. Crossing 1.10 is an investigation threshold,
...
infrastructure and its absolute cost is reasonable for the curated operation.
Every accepted exception must record absolute and relative measurements, stage
attribution, and the design tradeoff. A material, unexplained, or compounding
regression remains a failure. Compiler latency must not be reduced by weakening
correctness checks that belong on the production trust boundary or by overfitting
dispatch to the benchmark's formats, expressions, or kernels.
...
Shadow compilation is also excluded from release latency because it is confined
to a curated differential matrix, not the ordinary JIT or correctness suite.
...
- Baseline p50/p95 frontend-to-`KernelBuildSpec` latency on the curated corpus.
...
- Both reference machines have a reproducible generated-kernel baseline and A/A
  noise-floor calibration for phase-local comparisons.
...
- Analysis side tables for parents, uses, free/reduction variables, and accesses.
...
- Frozen `ScheduledCIN(cin, plan)` transitional carrier, with no node schema or
  attached analyses.
- Internal `LoopPlan` translated from public `Schedule`.
- Replacement of dynamic schedule attributes.
- Initial pass manager and pure analysis runner; no result cache or
...
- Isolated plumbing p95 satisfies the 1 ms ceiling. Production-mode
  ...
  above.
- Each extracted optimization that changes emitted code satisfies the
  phase-local two-machine generated-kernel gate in the same PR.
...
- Kernel signatures and result assembly are constructed outside semantic
  lowering.
- Every emission-affecting extraction satisfies the two-machine generated-kernel
  gate; production compiler latency remains within the Phase 0 target or has a
...
This phase occurs before the production LoopIR schema is frozen. It tests the
riskiest assumption in the architecture: that sparse merge semantics can be
represented by a clean one-directional lowering instead of the current recursive
...
- A deliberately small test/debug LoopIR interpreter (approximately 200 lines is
  a target, not a correctness constraint) independent of C++ emission.
- Differential tests against PyTorch or a simple reference across empty inputs,
  empty rows, unequal sparsity, ragged structures, and randomized patterns.
- A schema review documenting how merge selection, coordinate resolution,
  ...
  callbacks into a general lowerer or rendered-code parsing.

#### Go criteria

- Both examples execute correctly through the interpreter.
- The candidate IR contains no C++ spelling, regex-discovered semantics, mutable
  lowerer callbacks, or operation-specific escape hatch.
...
- The interpreter and lowering model remain simple enough to serve as an
  independent semantic oracle during migration.
- Phase 0-3 generated-kernel gates remain green; compiler-latency measurements
  are reviewed with no material or unexplained regression; and the
...
revise the LoopIR model or investment case, and repeat the review. This is a
...
- Curated shadow compilation comparing legacy and LoopIR paths; disabled by
  default outside dedicated debug/nightly workflows.
...
- Shadow comparison covers generated structure and numerics.
...
- `LoopPlan` encodings and typed transformations for representative tile-j and
  tile-ijk schedules, with no algorithm-specific handwritten C++ in the new path.

#### Exit criteria

- Schedule decisions are applied only to LoopIR for migrated operations.
- Panel/relayout/result-tile passes perform no name/regex discovery.
- Schedule cache identity comes from canonical `LoopPlan`.
- Explicit and auto schedules pass structural and numerical differential tests.
- Tile-j/tile-ijk generated kernels are ready for the Phase 8.5 selector-level
  parity validation against the corresponding prebuilt implementations after
  target lowering.
...
- LoopIR pipeline becomes default for every declared supported operation/format.
- Legacy compiler available temporarily behind a diagnostic fallback flag.
- Differential telemetry or test evidence for fallback frequency.
...
- Supported compatibility matrix is green through the new pipeline.
- No production test requires the legacy pipeline.
- Every migrated path satisfies the generated-kernel gate and compile-latency
  measurement/review protocol defined above. The cutover comparison is the new
  pipeline against legacy generated kernels, neutral-or-better across the full
  grid on both M5 and x86/redwood; parity with handwritten prebuilt kernels is
  deliberately not a cutover criterion (see Phase 8.5).
- Fallback is unused across the agreed validation suite.
- Legacy deletion reduces rather than merely relocates complexity.

### Phase 8.5: North-star validation against prebuilt schedules

This milestone measures the investment payoff. It runs after cutover and does
not gate legacy deletion: the legacy compiler is even further from the
handwritten kernels than the new pipeline is, so retaining it cannot help close
any gap found here.
...
- The primary parity measurement at the system level: the runtime tiling
  ...
  kernels across the grid. Prebuilt schedules only execute on shapes where the
  selector's probe chooses them, so per-cell dominance on shapes the selector
  would never route to that schedule is not required.

#### Exit criteria

- Selector-level performance with generated kernels is neutral-or-better
  against selector-level performance with prebuilt kernels, under the same
  thresholds as the generated-kernel gate, on both machines.
- Any shortfall is documented per shape regime and triggers an explicit scope
  review: additional target-lowering passes, retaining specific prebuilt
  kernels for the affected regimes, or revising the north-star claim. No
  outcome of this review reopens legacy-compiler retention.
...
- Optional compiler reentrancy/concurrent-compilation tests where supported by
  an actual production use case; not a cutover gate.
...
During Phases 4 through 8, support three explicit modes for migrated paths:

1. Legacy: compile and execute the current pipeline.
2. Shadow: compile with both pipelines, execute the selected production path,
   compare structural fingerprints and optionally numerics in tests/debug mode.
3. New: compile and execute LoopIR pipeline, with explicit diagnostic fallback
   during rollout only.

Fallback must be observable and categorized. Silent fallback would hide missing
coverage and prevent legacy deletion.
...
- the concrete M5 and x86/redwood gates in the protocol above, run in the PR that
  changes emission and not deferred to final cutover.
...
- generated tile-j and tile-ijk proof schedules meet selector-level
  handwritten-prebuilt parity on both M5 and x86/redwood in the Phase 8.5
  milestone, independent of cutover;
...
- generated kernel correctness and the concrete performance gates remain green.
...
1. Whether LoopIR uses nested regions, a block/CFG model, or a limited structured
   control-flow form. Structured regions are the preferred starting point.
2. Whether a distinct target-neutral KernelIR is needed between LoopIR and CxxIR.
   Do not add it until multiple target lowerings demonstrate the need.
3. How much semantic `Where` remains in normalized CIN versus becoming an
   explicit workspace/reduction plan.
4. Stable index-width policy and how int32/int64 choices enter type analysis and
   ABI lowering.
5. Multi-result support: explicitly reject initially or design first-class result
   tuples before enabling it.
...
do not freeze or productionize LoopIR until the Phase 3.5 sparse-intersection and
sparse-union spike passes its go/no-go review. The LoopIR phases above define
where the project is going and how to recognize when each migration step is
complete.
