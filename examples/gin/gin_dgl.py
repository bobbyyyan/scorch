...
# Define GIN layer
class GINLayer(dglnn.pytorch.conv.GINConv):
    def __init__(self, in_feats, out_feats, aggregator_type):
        super(GINLayer, self).__init__(
            apply_func=nn.Linear(in_feats, out_feats), aggregator_type=aggregator_type
        )


# Define GIN model using DGL
class DGLGIN(torch.nn.Module):
    def __init__(self, num_features, num_classes):
        super(DGLGIN, self).__init__()
        self.conv1 = GINLayer(num_features, 16, "sum")
        self.conv2 = GINLayer(16, num_classes, "sum")
...
model_dgl = DGLGIN(dataset.num_features, dataset.num_classes).to(torch.device("cpu"))
...
for key, value in state_dict.items():
    new_key = key.replace(".mlp.", ".apply_func.")
    new_value = value
