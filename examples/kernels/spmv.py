...
for matrix in tqdm(matrices, desc="Benchmarking SpMV"):
    try:
        ...
        dense_matrix = torch.rand((torch_sparse_matrix.shape[1],), dtype=torch.float32)
        ...
        for framework in ["PyTorch", "Scorch"]:
            ...
            for _ in range(10):
                if framework == "PyTorch":
                    ...
                    result = torch.matmul(torch_sparse_matrix, dense_matrix)
...
results_df.to_csv("spmv_benchmark_results.csv", index=False)
print("Benchmarking complete. Results saved to 'spmv_benchmark_results.csv'.")
...
plt.xscale('log')
plt.yscale('log')
plt.savefig("figures/spmv_benchmark_plot.pdf", bbox_inches='tight')
plt.show()
